...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
	...
	__parsec_bench_begin(__parsec_swaptions);
	  ...
	  if (!strcmp("-sm", argv[j])) {NUM_TRIALS = atoi(argv[++j]);}
        ...
        swaption_seed = (long)(double)(2147483647L * RanUnif(&seed));
	...
	factors[2][9]= -.001250;
	  ...
	  (parm *)malloc(sizeof(parm)*nSwaptions);
        ...
        for (i = 0; i < nSwaptions; i++) {
          ...
          swaptions[i].dYears = 5.0 + ((int)(double)(60*RanUnif(&seed)))*0.25; //5 to 20 years in 3 month intervals

          swaptions[i].dStrike = 0.1 + ((int)(double)(49*RanUnif(&seed)))*0.1; //strikes ranging from 0.1 to 5.0 in steps of 0.1
