...
# define FLOAT Fp16
#endif
# define FABS(x) FLOAT::abs((x))
# define FLOOR(x) FLOAT(floor(static_cast<double>((x))))
# define LOG(x) FLOAT(log(static_cast<double>((x))))
# define POW(x,y) FLOAT(pow(static_cast<double>((x)), static_cast<double>((y))))
...
#endif
#endif
#define FLOATString__(s) #s
#define FLOATString_(s) FLOATString__(s)
#define FLOATString FLOATString_(FLOAT)
...
*/
{
  /* Move constant initialization after static init */
  Zero = 0.0;
  Half = 0.5;
  One = 1.0;
  Two = 2.0;
...
/* .. if(-1)^N is invalid, replace MinusOne by One. */
  ...
  N = 0;
  Z = A1;
  M = (int)(double)FLOOR(Two * LOG(W) / LOG(A1));
  Break = False;
  do  {
    X = Z;
    I = 1;
...
*/
{
  static const char *const msg[] = { "FAILURE", "SERIOUS DEFECT", "DEFECT", "FLAW" };
...
void msglist ( const char*const* s )
